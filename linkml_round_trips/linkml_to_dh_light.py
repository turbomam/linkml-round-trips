...
#     SchemaDefinition, EnumDefinition
# )
...
for i in relevant_slots:
    slotname = i.name
    relevant_isa = i.is_a
    if relevant_isa is None:
        relevant_isa = default_section
...
for i in term_names:
    ...
    # not getting ontology IDs from NMDC... generate yaml first?
    ...
    # export menu saves a file but not with the briefer LinkML names (as opposed to titles)
...
final_frame.to_csv(output_file, sep="\t", index=False)

print(pd.Series(range_tally).value_counts())
